...
function Sidebar({ role }) {
    ...
    return (
        <motion.aside {...pageTransition} className="bg-pgray-light-3 p-4 shadow-inner">
            <h2 className="text-xl font-bold text-pgray-dark-2 mb-4">{role === 'admin' ? 'Admin Panel' : 'User Panel'}</h2>
            <ul className="space-y-2">
                {links.map((link) => (
                    <li key={link.path}>
                        <Link to={link} path className="block py-2 px-4 hover:bg-pgray-light-4 rounded">
                            {link.label}
                        </Link>
                    </li>
                ))}
