...
const createEducation = async (request, h) => {
    try {
        const { title, content, date, category, image } = request.payload;
        if (!title || !content || !date) {
            return h.response({ status: 'gagal', pesan: 'Judul, konten, dan tanggal wajib diisi' }).code(400).header('Cache-Control', 'no-store');
        }

        const newEducation = new Education({ id: nanoid(10), title, content, date, category, image, updatedAt: new Date() });
